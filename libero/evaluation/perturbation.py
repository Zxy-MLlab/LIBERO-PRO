"""Evaluation-facing BDDL perturbation and init-state preparation tool.

The perturbator implementations originate from the repository's root
``perturbation.py`` example. This module owns the stable evaluation API and has no
script entry point.
"""
...
class EvaluationSuite:
    ...
    def as_dict(self):
        return {
            "base_suite": self.base_suite,
            "effective_suite": self.effective_suite,
            "perturbation_type": self.perturbation_type,
        }


def resolve_evaluation_suite(configs: dict) -> Tuple[str, str, str]:
    """Return (base suite, generated suite, perturbation label) for this config."""
    base_suite = str(configs.get("task_suite_name", "")).strip()
    if not base_suite:
        raise ValueError("evaluation config must define task_suite_name")
    enabled = [flag for flag in PERTURBATION_FLAG_NAMES if bool(configs.get(flag, False))]
    if not enabled:
        return base_suite, base_suite, "none"
    if "use_task" in enabled and len(enabled) > 1:
        raise ValueError("use_task=True cannot be combined with other perturbations")
    if len(enabled) > 1:
        return base_suite, base_suite + "_temp", "+".join(PERTURBATION_FLAG_NAMES[x] for x in enabled)
    flag = enabled[0]
    suffix = str((configs.get("perturbation_mapping", {}) or {}).get(flag, "")).strip()
    if not suffix:
        raise ValueError("evaluation config has no perturbation_mapping for {}".format(flag))
    return base_suite, "{}_{}".format(base_suite, suffix), PERTURBATION_FLAG_NAMES[flag]


def _suite_data_complete(configs: dict, base_suite: str, effective_suite: str) -> Tuple[bool, List[str]]:
    bddl_root = Path(configs.get("bddl_files_path", ""))
    if bddl_root.name == base_suite:
        bddl_root = bddl_root.parent
    source_dir = bddl_root / base_suite
    target_dir = bddl_root / effective_suite
    init_dir = Path(configs.get("init_file_dir", "")) / effective_suite
    source_files = sorted(source_dir.glob("*.bddl"))
    if not source_files:
        raise FileNotFoundError("base suite has no BDDL files: {}".format(source_dir))
    missing = []
    for source in source_files:
        bddl = target_dir / source.name
        init_state = init_dir / (source.stem + ".pruned_init")
        if not bddl.is_file(): missing.append(str(bddl))
        if not init_state.is_file(): missing.append(str(init_state))
    return not missing, missing


def prepare_evaluation(config_path: str) -> EvaluationSuite:
    """Load one external YAML, generate missing data, and return its final suite."""
    path = Path(config_path).expanduser()
    if not path.is_file():
        raise FileNotFoundError("evaluation config not found: {}".format(path))
    with path.open("r", encoding="utf-8") as stream:
        configs = yaml.safe_load(stream) or {}
    base_suite, effective_suite, perturbation_type = resolve_evaluation_suite(configs)
    complete, missing = _suite_data_complete(configs, base_suite, effective_suite)
    if not complete:
        if perturbation_type == "none":
            raise FileNotFoundError("suite data is incomplete: " + ", ".join(missing))
        if not any(item.endswith(".bddl") for item in missing):
            bddl_root = Path(configs.get("bddl_files_path", ""))
            if bddl_root.name == base_suite:
                bddl_root = bddl_root.parent
            EvalEnvCreator(
                input_dir=str(bddl_root / effective_suite),
                script_path=configs.get("script_path", ""),
                base_output_dir=configs.get("init_file_dir", ""),
            ).create_env()
        else:
            generated_suite = create_env(configs)
            if generated_suite != effective_suite:
                raise RuntimeError("generator returned suite {!r}; expected {!r}".format(
                    generated_suite, effective_suite))
        complete, missing = _suite_data_complete(configs, base_suite, effective_suite)
        if not complete:
            raise FileNotFoundError("generated suite data is incomplete: " + ", ".join(missing))
    return EvaluationSuite(base_suite, effective_suite, perturbation_type)
...
class EvalEnvCreator:
    ...
    def create_env(self):
        ...
        # The generator is executed as a script from notebooks/, so Python would
        # otherwise search that directory instead of the repository root and fail
        # to import the local ``libero`` package.
        ...
        numba_cache = repo_root / ".runtime" / "numba_cache"
        numba_cache.mkdir(parents=True, exist_ok=True)
        child_env.setdefault("NUMBA_CACHE_DIR", str(numba_cache))
        matplotlib_cache = repo_root / ".runtime" / "matplotlib"
        matplotlib_cache.mkdir(parents=True, exist_ok=True)
        child_env.setdefault("MPLCONFIGDIR", str(matplotlib_cache))

        cmd = [
            sys.executable, str(script_path),
            "--bddl_base_dir", str(Path(self.input_dir).resolve()),
...
def process_bddl_file_mixed(input_dir: str,
                            ...
                            output_suite_name: Optional[str] = None,
    ...
    output_dir = input_path.parent / (output_suite_name or f"{input_path.name}_temp")
...
def create_env(
    ...
    """
    base_suite, output_suite, _ = resolve_evaluation_suite(configs)
    if output_suite == base_suite:
        return output_suite
    ...
    # 生成临时的 bddl 输出路径
    input_dir = Path(configs.get("bddl_files_path", ""))
    if input_dir.name != base_suite:
        input_dir = input_dir / base_suite
    temp_output_dir = process_bddl_file_mixed(
        input_dir=str(input_dir),
        task_suite_name=base_suite,
        flags=flags,
        configs=ood_task_configs,
        output_suite_name=output_suite,
        seed=configs.get("seed", 28),
    )
